...
class LightGCN(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        self.attention_layer = nn.Linear(self.embedding_dim * 4, 3) 
    ...
    def forward(self):
        ...
        # attention         
        h_v = self.item_id_embedding.weight.clone().detach()
        for i in range(self.n_layers):
            h_v = torch.sparse.mm(self.image_adj.cuda(), h_v)

        h_t = self.item_id_embedding.weight.clone().detach()
        for i in range(self.n_layers):
            h_t = torch.sparse.mm(self.text_adj.cuda(), h_t)

        h_s = self.item_id_embedding.weight.clone().detach()
        for i in range(self.n_layers):
            h_s = torch.sparse.mm(self.session_adj.cuda(), h_s)

        h_i = i_g_embeddings.clone().detach()

        concat_features = torch.cat([h_i, h_v, h_t, h_s], dim=-1)
        modality_weights = torch.sigmoid(self.attention_layer(concat_features)) # 
        modality_weights = F.softmax(modality_weights, dim=-1)
        

        global_weights = torch.mean(modality_weights, dim=0) #, keepdim=True)

        w_v = global_weights[0]  # 
        w_t = global_weights[1]  # 
        w_b = global_weights[2]  #

        mm_adj = (w_v * self.image_adj + w_t * self.text_adj + w_b * self.session_adj)
