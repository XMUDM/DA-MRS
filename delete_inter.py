...
for name in names:
    ...
    num_rows_to_delete = int(num_train * ratio)
   
    rows_to_delete = np.random.choice(train_df.index, num_rows_to_delete, replace=False)
    #print(type(rows_to_delete), rows_to_delete)
        ...
        if i_id not in train_dict[u_id]:
            if ((u_id not in test_dict) and (u_id not in valid_dict)):
                ...
                if ((i_id not in test_dict[u_id]) and (i_id not in valid_dict[u_id])):
                    ...
                    count += 1
    '''
    df.to_csv('./data/' + name + '/' + name + '_del_' + str(ratio) + '.inter', sep='\t', index=False)
