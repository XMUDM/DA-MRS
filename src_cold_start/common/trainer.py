...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    def __init__(self, config, model):
        ...
        self.best_test_topu_valid = tmp_dd
        self.best_test_bottomu_valid = tmp_dd
    ...
    def fit(self, train_data, valid_data=None, test_data=None, saved=False, save_dir=None, verbose=True, top_u=None, bottom_u=None):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                # test
                _, top_u_result = self._valid_epoch(top_u)
                _, bottom_u_result = self._valid_epoch(bottom_u)
                if verbose:
                    self.logger.info(valid_score_output)
                    self.logger.info(valid_result_output)
                    self.logger.info('topu result: \n' + dict2str(top_u_result))
                    self.logger.info('bottomu result: \n' + dict2str(bottom_u_result))
                if update_flag:
                    update_output = '██ ' + self.config['model'] + '--Best validation results updated!!!'
                    if verbose:
                        self.logger.info(update_output)
                    self.best_valid_result = valid_result                  
                    self.best_test_topu_valid = top_u_result
                    self.best_test_bottomu_valid = bottom_u_result
                if stop_flag:
                    ...
                    break
        return self.best_valid_score, self.best_valid_result, self.best_test_topu_valid, self.best_test_bottomu_valid
