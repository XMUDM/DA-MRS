...
class MFBPR(GeneralRecommender):
    ...
    def __init__(self, config, dataloader):
        super(MFBPR, self).__init__(config, dataloader)
        ...
        self.u_embedding = nn.Parameter(nn.init.xavier_uniform_(torch.empty(self.n_users, 2 * self.u_embedding_size)))
        self.i_embedding = nn.Parameter(nn.init.xavier_uniform_(torch.empty(self.n_items, 2 * self.i_embedding_size)))
