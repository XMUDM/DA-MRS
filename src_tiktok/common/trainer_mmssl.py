...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None):
        ...
        total_loss = None
        loss_batches = []
        for batch_idx, interaction in enumerate(train_data):  #interaction (3,batch-size) 3: user,pos_item,neg_item
            #self.optimizer.zero_grad()
            if interaction.shape[1] != train_data.batch_size:
                continue
            losses = loss_func(interaction)  # 计算loss
    ...
    def evaluate(self, eval_data, is_test=False, idx=0):
        ...
        for batch_idx, batched_data in enumerate(eval_data):
            ...
            scores = self.model.full_sort_predict(batched_data) #batch-size 用户对所有item的评分
