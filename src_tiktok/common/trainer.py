   ...
   and some other features helpful for model training and evaluation.
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None):
        ...
        for batch_idx, interaction in enumerate(train_data):
            ...
            # ## get mask item
            # _, item_idx = torch.topk(torch.tensor(list(self.item_easy.values())), 100, dim=0, largest=True, sorted=True)

            # mask = torch.ones(self.model.n_items)
            # mask[item_idx] = 0
            
            # # # mask 为 1，即被mask的部分使用
            # # mask = torch.zeros(self.model.n_items)
            # # mask[item_idx] = 1

            losses = loss_func(interaction)# , batch_idx) # , mask)
            # # update item
            # # 选择loss_each中top10的元素所在的索引
            # # loss 越小，表示学的越好，largest = false, 等于找到学的最好的 K 个
            # v, idx = torch.topk(upd_loss, 100, dim=0, largest=False, sorted=True)
            # # 更新pool
            # for i in idx:
            #     self.item_easy[i.item()] += 1
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                if update_flag:
                    ...
                    # if saved:
                    #     # save_dir = save_path # '/data1/gpxu/MM_pre/src/log/' + param
    ...
    def evaluate(self, eval_data, is_test=False, idx=0):
        ...
        # upd
        # _, item_idx = torch.topk(torch.tensor(list(self.item_easy.values())), 100, dim=0, largest=True, sorted=True) ## 找到难度最小的item
