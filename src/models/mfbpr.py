...
class MFBPR(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        self.image_adj, self.text_adj = self.get_knn_adj_mat(self.image_embedding.weight.detach(), self.text_embedding.weight.detach())
    ...
    def calculate_loss(self, interaction):
        ...
        return batch_mf_loss + self.neighbor_weight * (neighbor_dis_loss) + KL_loss * self.kl_weight + self.reg_weight * reg_loss
    ...
    def full_sort_predict(self, interaction):
        ...
        all_item_e = item_embeddings + (h_s+h_v+h_t)/3.0
    ...
    def get_weight_modal(self, users, pos_items, neg_items, user_embeddings, h_t, h_v, h_s):
        ...
        p_mean_probability = torch.pow(p_mean_value, 1).data
        p_var_probability = torch.pow(torch.exp(-p_variance).data, 2) # 0 ~ 1 
